...
if [ -z "$VO" ]; then
  echo "Usage: $0 SOURCE DEST VO"
  echo
  echo "Where SOURCE is an LFC namespace item, DEST is a local one, "
  echo "VO is your VO."
  exit 1
fi
  ...
  if [ "`echo -n $i|sed -e \"s/^\/.*$/\//\"`" = "/" ] # if the line starts with a / then
    ...
    if [ "`echo -n $i|sed -e \"s/^d.*$/d/\"`" = "d" ] # if directory in filelist
      ...
      /bin/true # do nothing
        ...
        echo "lcg-cp --vo hungrid lfn:$MYLASTDIR/$j file:$MYDESTDIR/$j" # copy it to wherever it would go
      fi
    fi
  fi
done
